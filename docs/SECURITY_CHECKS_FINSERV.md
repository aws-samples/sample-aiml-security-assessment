...
After consolidation the combined framework contains **87 upstream + 27 AG + 64 FS = 178 distinct checks** (down from 70 + 27 AG + 69 FS = 166 before merging FinServ overlaps; the upstream count grew from 70 to 87 across two correctness/gap-closure passes: PR-0 split SM-01/SM-03 into notebook-scoped and repo-specific checks, and a subsequent pass closed the remaining Security Hub AI Security Best Practices gaps — Bedrock.1, BedrockAgentCore.5/.7, and 12 SageMaker controls). The consolidation reduces duplication without losing FinServ-specific regulatory depth.
...
| Detection | Calls `bedrock:ListGuardrails`; if no guardrails are configured, reports N/A ("Relevance Grounding Not Applicable") rather than a failure, matching the same "nothing to check" convention used by FS-36/FS-38/FS-45/FS-47. When guardrails exist, calls `bedrock:GetGuardrail` and inspects `contextualGroundingPolicy.filters` for the `RELEVANCE` filter type. Flags guardrails with no relevance filter configured. |
