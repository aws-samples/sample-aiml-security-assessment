"""
Drift-guard for the Bedrock severity register
(docs/AI_SECURITY_BEST_PRACTICES_GAP_ANALYSIS.md "Severity Model").

Mirrors tests/test_agentcore_severity_register.py: the FinServ severity
methodology is adopted here for the Bedrock (BR-*) checks, seeding control
severity from the more defensible/most common severity across a check's own
Pass/Fail call sites (Bedrock has no single Security Hub control per BR-*
check the way AgentCore does for most of its checks; Bedrock.1 for BR-33 is
the one exception and its severity is Medium either way).
...
if _br_dir not in sys.path:
    sys.path.insert(0, _br_dir)
...
if "bedrock_app" in sys.modules:
    ...
    # any stale cache entries so the import below (and the one further down
    # in this file) resolves against _br_dir (already at the front of
    # sys.path).
    sys.modules.pop("severity_disposition", None)
    sys.modules.pop("schema", None)
    _spec = importlib.util.spec_from_file_location(
        "bedrock_app", os.path.join(_br_dir, "app.py")
    )
...
def _static_finding_names_from_source():
    ...
    above instead."""
    tree = ast.parse(inspect.getsource(bedrock_app))
    names = set()
