...
<body>
    ...
    <script>
        ...
        createServiceFilter('agenticTable', 'agenticSearchInput', 'agenticAccountFilter', 'agenticRegionFilter', 'agenticSeverityFilter', 'agenticStatusFilter', 'agenticResetFilters');
...
def generate_html_report(
    ...
    agentic_rows = generate_table_rows(
        service_findings.get("agentic", []), include_data_attrs=True
    )
    finserv_rows = generate_table_rows(
    ...
    if num_real_regions + (1 if has_global else 0) > 1:
        ...
        agentic_region_filter = f'<div class="filter-group"><label for="agenticRegionFilter">Region</label><select id="agenticRegionFilter"><option value="">All Regions</option>{region_options}</select></div>'
    else:
        region_filter = ""
        bedrock_region_filter = ""
        sagemaker_region_filter = ""
    ...
    if mode == "multi":
        ...
        agentic_account_filter = f'<div class="filter-group"><label>Account</label><select id="agenticAccountFilter"><option value="">All Accounts</option>{account_options}</select></div>'
        finserv_account_filter = f'<div class="filter-group"><label>Account</label><select id="finservAccountFilter"><option value="">All Accounts</option>{account_options}</select></div>'
    ...
    if agentic_total > 0:
        agentic_regions = sorted(
            {
                f.get("region", f.get("Region", ""))
                for f in service_findings.get("agentic", [])
                if f.get("region", f.get("Region", ""))
            }
        )
        agentic_region_options = "".join(
            [f'<option value="{r}">{r}</option>' for r in agentic_regions]
        )
        agentic_region_filter = (
            '<div class="filter-group"><label>Region</label>'
            '<select id="agenticRegionFilter">'
            f'<option value="">All Regions</option>{agentic_region_options}</select></div>'
            if agentic_regions
            else agentic_region_filter
        )
        agentic_nav = (
        ...
        agentic_section = (
            ...
            '<div class="muted" style="margin:-6px 0 10px;font-size:13px;">Scope: API-provable Agentic AI security controls mapped to the AWS Well-Architected Agentic AI Lens security guidance. '
            "Human-in-the-loop governance is referenced in methodology but not scored automatically unless an AWS API can prove the control.</div>"
            '<div class="filter-bar">'
            '<div class="filter-group"><label>Search</label><input type="text" placeholder="Search findings..." id="agenticSearchInput"></div>'
            + agentic_account_filter
            + agentic_region_filter
            + '<div class="filter-group"><label>Severity</label><select id="agenticSeverityFilter"><option value="">All Severities</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option><option value="informational">Informational</option></select></div>'
            '<div class="filter-group"><label>Status</label><select id="agenticStatusFilter"><option value="">All Statuses</option><option value="failed">Failed</option><option value="passed">Passed</option><option value="n/a">N/A</option></select></div>'
            '<button class="btn btn-reset" id="agenticResetFilters"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>Reset</button>'
            "</div>"
            '<div class="card"><div class="table-wrap"><table id="agenticTable"><thead><tr><th>Account ID</th><th>Region</th><th>Check ID</th><th>Finding</th><th>Details</th><th>Resolution</th><th>Reference</th><th>Severity</th><th>Status</th></tr></thead><tbody>'
            + agentic_rows
            + "</tbody></table></div></div>"
            "</section>"
        )
    ...
    if finserv_total > 0:
        ...
        finserv_section = (
            ...
            "</div>"
            '<div class="card"><div class="table-wrap"><table id="finservTable"><thead><tr><th>Account ID</th><th>Region</th><th>Check ID</th><th>Finding</th><th>Details</th><th>Resolution</th><th>Reference</th><th>Severity</th><th>Status</th></tr></thead><tbody>'
            + finserv_rows
            + "</tbody></table></div></div>"
            "</section>"
        )
    ...
    rendered_html = html_template.format(
        ...
        agentic_total=service_stats.get("agentic", {}).get("passed", 0)
        + service_stats.get("agentic", {}).get("failed", 0)
        + service_stats.get("agentic", {}).get("na", 0),
        agentic_failed=service_stats.get("agentic", {}).get("failed", 0),
        agentic_passed=service_stats.get("agentic", {}).get("passed", 0),
        ...
        agentic_account_filter=agentic_account_filter,
